...
input:
{
  "image_url": "https://...png",       # обязательный — оживляем эту картинку
  "prompt": "...",                       # опц. промт движения
  "negative_prompt": "...",              # опц.
  "width": 480, "height": 832,           # кратно 32 (вертикаль по умолчанию)
  "num_frames": 97,                      # кратно 8 +1
  "steps": 40,                           # шагов диффузии (base ~40)
  "fps": 24,
  "seed": 0
}
...
# HF-кэш: network volume если примонтирован, иначе локально в контейнере
...
def get_pipe():
    global _pipe
    if _pipe is None:
        p = LTXImageToVideoPipeline.from_pretrained(MODEL, torch_dtype=torch.bfloat16)
        p.to("cuda")
        try:
            p.vae.enable_tiling()
        except Exception:
...
def handler(job):
    ...
    try:
        ...
        return {"error": "i2v failed", "detail": str(e)[-600:]}
